...
class NuScenesTrajectories(SingleAgentDataset):
    ...
    def __init__(self, mode: str, data_dir: str, args: Dict, helper: PredictHelper):
        ...
        # In load_data mode, drop any tokens whose pickle isn't on disk —
        # useful when extraction was interrupted partway through a split.
        if mode == 'load_data':
            existing = {f[:-len('.pickle')] for f in os.listdir(data_dir)
                        if f.endswith('.pickle') and f != 'stats.pickle'}
            before = len(self.token_list)
            self.token_list = [t for t in self.token_list if t in existing]
            dropped = before - len(self.token_list)
            if dropped:
                print(f'[{type(self).__name__}/{args["split"]}] dropped {dropped} tokens '
                      f'without on-disk pickles ({len(self.token_list)} remain)')
    ...
    def save_data(self, idx: int, data: Dict):
        ...
        if os.path.isfile(filename):  # skip-existing — lets us resume after a crash
            return
