...
class Trainer:
    ...
    def load_checkpoint(self, checkpoint_path, just_weights=False):
        ...
        # With just_weights we allow new params in the current model that aren't in
        # the checkpoint (e.g. zero-init text_proj in LVMRankedText fine-tuning).
        missing, unexpected = self.model.load_state_dict(
            checkpoint['model_state_dict'], strict=not just_weights)
        if just_weights and (missing or unexpected):
            print(f"[load_checkpoint] missing in ckpt: {len(missing)}  "
                  f"unexpected in ckpt: {len(unexpected)}")
            if missing:
                print(f"  missing  (will keep model init): {missing[:6]}{'...' if len(missing)>6 else ''}")
            if unexpected:
                print(f"  unexpected (ignored): {unexpected[:6]}{'...' if len(unexpected)>6 else ''}")
        if not just_weights:
